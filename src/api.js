...
router.post('/books', function (req, res) {
    ...
    let sess = req.session;
    if(!sess.email){res.send("You must be logged in to add a new book.");}
    const {
        title,
        isbn,
        pageCount,
...
router.put('/books/:id', function (req, res) {
    let sess = req.session;
    if(!sess.email){res.send("You must be logged in to edit a book.");}
    const { id } = req.params;
    const {
        title,
        isbn,
...
router.delete('/books/:id', function (req, res) {
    let sess = req.session;
    if(!sess.email){res.send("You must be logged in to delete book.");}
    const { id } = req.params;
